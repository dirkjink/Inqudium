 ...
 *
 * <p>Uses {@link java.util.concurrent.Semaphore} which is virtual-thread safe
 * (backed by {@code AbstractQueuedSynchronizer} → {@code LockSupport.park},
 * no carrier-thread pinning) (ADR-008, ADR-020).
 *
 * <p>The {@code finally} block guarantees permit release on every code path.
 *
 * @since 0.1.0
 */
public final class SemaphoreBulkhead implements Bulkhead {
    ...
    public SemaphoreBulkhead(String name, BulkheadConfig config) {
        ...
        this.semaphore = new Semaphore(config.getMaxConcurrentCalls(), true); // fair ordering
    }
    ...
    public int getConcurrentCalls() {
        return config.getMaxConcurrentCalls() - semaphore.availablePermits();
    }
    ...
    private void acquirePermit(String callId) {
        ...
        if (!acquired) {
            ...
            eventPublisher.publish(new BulkheadOnRejectEvent(
                    callId, name, concurrent, Instant.now()));
        ...
        eventPublisher.publish(new BulkheadOnAcquireEvent(
                callId, name, getConcurrentCalls(), Instant.now()));
    }
