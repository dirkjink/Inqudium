...
public final class TimeLimiterImpl implements TimeLimiter {
  ...
  private <T> T executeFuture(String callId, Supplier<CompletionStage<T>> futureSupplier) {
    ...
    try {
      ...
      eventPublisher.publish(new TimeLimiterOnErrorEvent(callId, name, duration, e, Instant.now()));
      throw e;
    }
    ...
    try {
      ...
      var duration = Duration.between(start, Instant.now());
      eventPublisher.publish(new TimeLimiterOnSuccessEvent(callId, name, duration, Instant.now()));
      return result;
    } catch (TimeoutException te) {
      var actualDuration = Duration.between(start, Instant.now());
      eventPublisher.publish(new TimeLimiterOnTimeoutEvent(callId, name, timeout, Instant.now()));
      ...
      eventPublisher.publish(new TimeLimiterOnErrorEvent(callId, name, duration, cause, Instant.now()));
    ...
    future.whenComplete((result, throwable) -> {
      ...
      try {
        ...
        System.getLogger(TimeLimiterImpl.class.getName())
            .log(System.Logger.Level.WARNING, "Orphaned call handler threw: {0}", e.getMessage());
      }
    });
  }
}
