...
/**
 * Imperative time limiter implementation.
 *
 * <p>Uses {@link CompletableFuture#get(long, TimeUnit)} for timeout enforcement.
 * Synchronous suppliers are executed on a virtual thread via
 * {@link CompletableFuture#supplyAsync(Supplier, Executor)} (ADR-008, ADR-010).
 *
 * <p>No thread interrupts — orphaned operations continue to completion.
 * Orphaned results/errors are forwarded to the configured handlers.
 *
 * @since 0.1.0
 */
public final class TimeLimiterImpl implements TimeLimiter {
        ...
        return () -> {
            ...
            try {
                ...
                eventPublisher.publish(new TimeLimiterOnErrorEvent(
                        callId, name, duration, e, Instant.now()));
                throw e;
            }
            ...
            try {
                ...
                eventPublisher.publish(new TimeLimiterOnSuccessEvent(
                        callId, name, duration, Instant.now()));
                return result;
            } catch (TimeoutException te) {
                var actualDuration = Duration.between(start, Instant.now());
                eventPublisher.publish(new TimeLimiterOnTimeoutEvent(
                        callId, name, timeout, Instant.now()));

                // Install orphaned call handlers — no cancel, no interrupt (ADR-010)
                ...
                eventPublisher.publish(new TimeLimiterOnErrorEvent(
                        callId, name, duration, cause, Instant.now()));
    ...
    public <T> Supplier<T> decorateSupplier(Supplier<T> supplier) {
        // Wrap synchronous supplier in a CompletableFuture on a virtual thread
    ...
    private <T> void installOrphanedHandlers(CompletableFuture<T> future, String callId, Instant start) {
        ...
        if (onResult == null && onError == null) {
            return; // no handlers configured — orphaned call runs silently
        }
        ...
        future.whenComplete((result, throwable) -> {
            ...
            try {
                if (throwable != null) {
                    ...
                    if (onResult != null) {
                        onResult.accept(ctx, result);
                    }
                }
            } catch (Exception e) {
                // Orphaned handler exceptions are swallowed
                System.getLogger(TimeLimiterImpl.class.getName())
                        .log(System.Logger.Level.WARNING,
                                "Orphaned call handler threw: {0}", e.getMessage());
            }
        });
    }
}
