 ...
 * else (chain ID, call ID) uses primitives.</p>
 *
 ...
 *
 * // Direct execution — no wrapper objects created
 * String result = bulkhead.executeSupplier(() -> fetchData());
 * int count     = bulkhead.executeCallable(() -> readFile());
 *                 bulkhead.executeRunnable(() -> sendEmail());
 * int length    = bulkhead.executeFunction(String::length, "hello");
 * }</pre>
 *
...
public interface InqExecutor<A, R> extends LayerAction<A, R> {
   ...
   *
   * <p>Intended for executors with type parameters {@code <Void, Void>}.</p>
   *
   ...
   * <p>Intended for executors with type parameters {@code <Void, T>}.</p>
   *
   * @param execution the proxy operation to execute
